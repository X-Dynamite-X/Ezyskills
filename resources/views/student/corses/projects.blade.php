<section class="py-8 px-6 w-full">
   ...
   @foreach ($enrollment->course->courseInfo->projects as $projectTitle => $projectDetails)
    <div
        class="project-card bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 cursor-pointer transform hover:-translate-y-1">
        <div class="flex items-center p-4">
            ...
            <div class="flex-grow">
                <h3 class="text-base font-semibold text-gray-800">{{ $projectTitle }}</h3>
                @foreach ($projectDetails as $detail)
                    <p class="text-gray-500 text-xs mt-1 project-details hidden">{{ $detail }}</p>
                @endforeach
            </div>
